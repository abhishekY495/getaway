...
import { useVenueCityEvents } from "@/lib/hooks/events/use-get-venue-events";

export default function VenueEventsScreen() {
  const { venueId } = useLocalSearchParams();
  const { data, isLoading } = useVenueCityEvents(Number(venueId));
  ...
  return (
    <ScrollView className="mb-5 bg-white">
      <View className="relative">
        <Image
          source={result.venue.coverImage}
        ...
        <Text className="font-black text-2xl absolute text-white bottom-4 left-5">
          Popular experiences in {result.venue.name}
        </Text>
      </View>
      <View className="mx-1">
